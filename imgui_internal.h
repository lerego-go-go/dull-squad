 ...
 */
#define sw_swapcode(TYPE, parmi, parmj, n) { 		\
...
{
    if (swaptype <= 1)
        sw_swapcode(long, a, b, n)
    else
        sw_swapcode(char, a, b, n)
}
...
#define sw_vecswap(a, b, n) 	if ((n) > 0) swapfunc(a, b, n, swaptype)
...
static __inline void
cr_im_qsort(void* aa, size_t n, size_t es, int (*cmp)(const void*, const void*))
{
...
loop:	SWAPINIT(a, es);
    ...
    pn = (char*)a + n * es;
    r = sw_min(pa - (char*)a, pb - pa);
    sw_vecswap(a, pb - r, r);
    r = sw_min(pd - pc, pn - pd - (int)es);
    sw_vecswap(pb, pn - r, r);
    if ((r = pb - pa) > (int)es)
        cr_im_qsort(a, r / es, es, cmp);
    if ((r = pd - pc) > (int)es) {
...
static inline int cr_im_abs(int i)
{
    return i < 0 ? -i : i;
}

static inline double cr_im_fabs(double x)
{
    return x < 0 ? -x : x;
}

static inline float cr_im_fabsf(double x)
{
    return x < 0 ? -x : x;
}

static inline float cr_im_sqrtf(float x)
{
    return _mm_cvtss_f32(_mm_sqrt_ss(_mm_set_ss(x)));
}

static inline float cr_im_expf(float x)
{
    return _mm_cvtss_f32(_mm_exp_ps(_mm_set_ss(x)));
}

static inline float cr_im_sinf(float x)
{
    return _mm_cvtss_f32(_mm_sin_ps(_mm_set_ss(x)));
}

static inline float cr_im_cosf(float x)
{
    return _mm_cvtss_f32(_mm_cos_ps(_mm_set_ss(x)));
}

/*static inline float cr_im_tanf(float x)
{
    return _mm_cvtss_f32(_mm_tan_ps(_mm_set_ss(x)));
}*/

static inline float cr_im_atan(float x)
{
    return _mm_cvtss_f32(_mm_atan_ps(_mm_set_ss(x)));
}

static inline float cr_im_asinf(float x)
{
    return _mm_cvtss_f32(_mm_asin_ps(_mm_set_ss(x)));
}

static inline float cr_im_powf(float x, float y)
{
    return _mm_cvtss_f32(_mm_pow_ps(_mm_set_ss(x), _mm_set_ss(y)));
}

static inline float cr_im_acosf(float x)
{
    return _mm_cvtss_f32(_mm_acos_ps(_mm_set_ss(x)));
}

static inline float cr_im_logf(float x)
{
    return _mm_cvtss_f32(_mm_log_ps(_mm_set_ss(x)));
}

static inline int cr_im_ceil(float x)
{
    int i = (int)x;
    return x > (float)i ? i + 1 : i;
}

static inline float cr_im_atan2f(float x, float y)
{
    return _mm_cvtss_f32(_mm_atan2_ps(_mm_set_ss(x), _mm_set_ss(y)));
}

#define cr_isdigit(c) (c >= '0' && c <= '9')

static inline double cr_im_atof(const char* s)
{
    double a = 0.0;
    int e = 0;
    int c;
    while ((c = *s++) != '\0' && cr_isdigit(c)) {
        a = a * 10.0 + (c - '0');
    }
    if (c == '.') {
        while ((c = *s++) != '\0' && cr_isdigit(c)) {
...
#define ImFabs(X)           cr_im_fabsf(X)
#define ImSqrt(X)           cr_im_sqrtf(X)
//#define ImFmod(X, Y)        fmodf((X), (Y))
#define ImCos(X)            cr_im_cosf(X)
#define ImSin(X)            cr_im_sinf(X)
#define ImAcos(X)           cr_im_acosf(X)
#define ImAtof(STR)         cr_im_atof(STR)
#define ImAtan2(Y, X)       cr_im_atan2f((Y), (X))
//#define ImAtof(STR)         atof(STR)
//#define ImFloorStd(X)     floorf(X)           // We use our own, see ImFloor() and ImFloorSigned()
#define ImCeil(X)           cr_im_ceil(X)

#define FLOAT_NON_FRACTIONAL (8388608.f)

static inline float cr_imTruncToFloat(float F)
{
    return _mm_cvtss_f32(_mm_round_ps(_mm_set_ss(F), 3));
}

template<class T> static inline T im_cClamp(const T X, const T Min, const T Max)
{
...
static inline float ImFmod(float X, float Y)
{
    ...
    const float Quotient = cr_im_fabs(Div) < FLOAT_NON_FRACTIONAL ? cr_imTruncToFloat(Div) : Div;
    float IntPortion = Y * Quotient;

    if (cr_im_fabs(IntPortion) > cr_im_fabs(X))
    {
        IntPortion = X;
    }
    ...
    return im_cClamp(Result, -AbsY, AbsY);
}

static inline float  ImPow(float x, float y) { return cr_im_powf(x, y); }          // DragBehaviorT/SliderBehaviorT uses ImPow with either float/double and need the precision
static inline double ImPow(double x, double y) { return cr_im_powf(x, y); }
static inline float  ImLog(float x) { return cr_im_logf(x); }             // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and need the precision
static inline double ImLog(double x) { return cr_im_logf(x); }
static inline int    ImAbs(int x) { return x < 0 ? -x : x; }
static inline float  ImAbs(float x) { return (float)cr_im_fabs(x); }
static inline double ImAbs(double x) { return cr_im_fabs(x); }
static inline float  ImSign(float x) { return (x < 0.0f) ? -1.0f : ((x > 0.0f) ? 1.0f : 0.0f); } // Sign operator - returns -1, 0 or 1 based on sign of argument
static inline double ImSign(double x) { return (x < 0.0) ? -1.0 : ((x > 0.0) ? 1.0 : 0.0); }
#ifdef IMGUI_ENABLE_SSE
static inline float  ImRsqrt(float x) { return _mm_cvtss_f32(_mm_rsqrt_ss(_mm_set_ss(x))); }
#else
static inline float  ImRsqrt(float x) { return 1.0f / cr_im_sqrtf(x); }
#endif
static inline double ImRsqrt(double x) { return 1.0 / cr_im_sqrtf(x); }
#endif
...
struct IMGUI_API ImBitArray
{
    ...
    void            ClearAllBits()              { im_cr_memset(Storage, 0, sizeof(Storage)); }
    void            SetAllBits()                { im_cr_memset(Storage, 255, sizeof(Storage)); }
...
struct IMGUI_API ImBitVector
{
    ...
    void            Create(int sz)              { Storage.resize((sz + 31) >> 5); im_cr_memset(Storage.Data, 0, (size_t)Storage.Size * sizeof(Storage.Data[0])); }
...
struct ImSpanAllocator
{
    ...
    ImSpanAllocator()                               { im_cr_memset(this, 0, sizeof(*this)); }
...
struct IMGUI_API ImGuiComboPreviewData
{
    ...
    ImGuiComboPreviewData() { im_cr_memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiPopupData
{
    ...
    ImGuiPopupData()    { im_cr_memset(this, 0, sizeof(*this)); OpenFrameCount = -1; }
};
...
struct ImGuiNavItemData
{
    ...
    void Clear()        { Window = NULL; ID = FocusScopeId = 0; RectRel = ImRect(); DistBox = DistCenter = DistAxial = IM_FLT_MAX; }
};
...
struct ImGuiNextWindowData
{
    ...
    ImGuiNextWindowData()       { im_cr_memset(this, 0, sizeof(*this)); }
...
struct ImGuiNextItemData
{
    ...
    ImGuiNextItemData()         { im_cr_memset(this, 0, sizeof(*this)); }
...
struct ImGuiLastItemData
{
    ...
    ImGuiLastItemData()     { im_cr_memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiOldColumnData
{
    ...
    ImGuiOldColumnData() { im_cr_memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiOldColumns
{
    ...
    ImGuiOldColumns()   { im_cr_memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiWindowSettings
{
    ...
    ImGuiWindowSettings()       { im_cr_memset(this, 0, sizeof(*this)); }
...
struct IMGUI_API ImGuiStackSizes
{
    ...
    ImGuiStackSizes() { im_cr_memset(this, 0, sizeof(*this)); }
...
struct ImGuiContextHook
{
    ...
    ImGuiContextHook()          { im_cr_memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiContext
{
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas)
    {
        ...
        TabFocusRequestCurrCounterRegular = TabFocusRequestCurrCounterTabStop = IM_INT_MAX;
        TabFocusRequestNextCounterRegular = TabFocusRequestNextCounterTabStop = IM_INT_MAX;
        ...
        ColorEditLastColor[0] = ColorEditLastColor[1] = ColorEditLastColor[2] = IM_FLT_MAX;
        ...
        PlatformImePos = PlatformImeLastPos = ImVec2(IM_FLT_MAX, IM_FLT_MAX);
        ...
        im_cr_memset(FramerateSecPerFrame, 0, sizeof(FramerateSecPerFrame));
        FramerateSecPerFrameIdx = FramerateSecPerFrameCount = 0;
        FramerateSecPerFrameAccum = 0.0f;
        WantCaptureMouseNextFrame = WantCaptureKeyboardNextFrame = WantTextInputNextFrame = -1;
        im_cr_memset(TempBuffer, 0, sizeof(TempBuffer));
    }
};
...
struct ImGuiTabItem
{
    ...
    ImGuiTabItem()      { im_cr_memset(this, 0, sizeof(*this)); LastFrameVisible = LastFrameSelected = -1; NameOffset = -1; BeginOrder = IndexDuringLayout = -1; }
};
...
struct ImGuiTable
{
    ...
    IMGUI_API ImGuiTable()      { im_cr_memset(this, 0, sizeof(*this)); LastFrameActive = -1; }
...
struct ImGuiTableTempData
{
    ...
    IMGUI_API ImGuiTableTempData() { im_cr_memset(this, 0, sizeof(*this)); LastTimeActive = -1.0f; }
};
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableSettings()        { im_cr_memset(this, 0, sizeof(*this)); }
...
namespace ImGui
{
    ...
    IMGUI_API int           im_sscanf(const char* const destination, const char* const format, ...);
