...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (use_child_window)
    {
        ...
        ImVec2 override_content_size(IM_FLT_MAX, IM_FLT_MAX);
        if ((flags & ImGuiTableFlags_ScrollX) && !(flags & ImGuiTableFlags_ScrollY))
            override_content_size.y = IM_FLT_MIN;
        ...
        if (override_content_size.x != IM_FLT_MAX || override_content_size.y != IM_FLT_MAX)
            SetNextWindowContentSize(ImVec2(override_content_size.x != IM_FLT_MAX ? override_content_size.x : 0.0f, override_content_size.y != IM_FLT_MAX ? override_content_size.y : 0.0f));
...
void ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count)
{
    ...
    im_cr_memset(table->RawData, 0, span_allocator.GetArenaSizeInBytes());
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    {
        if (table->ResizedColumn != -1 && table->ResizedColumnNextWidth != IM_FLT_MAX)
            TableSetColumnWidth(table->ResizedColumn, table->ResizedColumnNextWidth);
        table->LastResizedColumn = table->ResizedColumn;
        table->ResizedColumnNextWidth = IM_FLT_MAX;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if ((table->EnabledMaskByDisplayOrder & ((ImU64)1 << order_n)) == 0)
        {
            ...
            column->ClipRect.Max.y = IM_FLT_MAX;
    ...
    {
        const ImGuiID context_menu_id = ImHashStr(xor ("##ContextMenu"), 0, table->ID);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (held)
        {
            if (table->LastResizedColumn == -1)
                table->ResizeLockMinContentsX2 = table->RightMostEnabledColumn != -1 ? table->Columns[table->RightMostEnabledColumn].MaxX : -IM_FLT_MAX;
...
void    ImGui::EndTable()
{
    ...
    // Pop from id stack
    IM_ASSERT_USER_ERROR(inner_window->IDStack.back() == table->ID + table->InstanceCurrent, xor ("fqegt"));
    IM_ASSERT_USER_ERROR(outer_window->DC.ItemWidthStack.Size >= temp_data->HostBackupItemWidthStackSize, xor ("qgqtz"));
    PopID();
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    {
        IM_ASSERT_USER_ERROR(table->DeclColumnsCount < table->ColumnsCount, xor ("tjknu"));
        return;
    }
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (merge_group->ChannelsCount == 0)
                merge_group->ClipRect = ImRect(+IM_FLT_MAX, +IM_FLT_MAX, -IM_FLT_MAX, -IM_FLT_MAX);
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for (int n = 0; n < splitter->_Count && merge_channels_count != 0; n++)
                {
                    ...
                    im_cr_memcpy(dst_tmp++, channel, sizeof(ImDrawChannel));
                }
            }

            // Make sure Bg2DrawChannelUnfrozen appears in the middle of our groups (whereas Bg0/Bg1 and Bg2 frozen are fixed to 0 and 1)
            if (merge_group_n == 1 && has_freeze_v)
                im_cr_memcpy(dst_tmp++, &splitter->_Channels[table->Bg2DrawChannelUnfrozen], sizeof(ImDrawChannel));
        }
        ...
        im_cr_memcpy(splitter->_Channels.Data + LEADING_DRAW_CHANNELS, g.DrawChannelsTempMergeBuffer.Data, (splitter->_Count - LEADING_DRAW_CHANNELS) * sizeof(ImDrawChannel));
    }
}
...
void ImGui::TableHeader(const char* label)
{
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        ...
        {
            ImFormatString(sort_order_suf, IM_ARRAYSIZE(sort_order_suf), xor ("%d"), column->SortOrder + 1);
...
void ImGui::TableGcCompactSettings()
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
        if (settings->ID != 0)
            im_cr_memcpy(new_chunk_stream.alloc_chunk(TableSettingsCalcChunkSize(settings->ColumnsCount)), settings, TableSettingsCalcChunkSize(settings->ColumnsCount));
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    ImGuiID id = window->GetID(str_id ? str_id : xor ("columns"));
    PopID();
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    for (int n = 0; n < columns_count; n++)
    {
        ...
        column->ClipRect = ImRect(clip_x1, -IM_FLT_MAX, clip_x2, +IM_FLT_MAX);
